...
app = typer.Typer(
    name="dpx-c",
    help="⚡ Hexagonal Pattern Scanner & Architecture Detector for Pure C (C89, C99, C11, C17, C23).",
...
def scan(
    path: Annotated[
        str,
        typer.Argument(
            help="Path to a C source/header file (.c / .h) or directory containing a C project/subsystem.",
        ),
    ] = ".",
    ...
    pattern: Annotated[
        ...
        typer.Option(
            "--pattern",
            "-p",
            help="Filter for specific pattern type(s) (e.g. 'vtable_interface', 'opaque_pointer_adt', 'intrusive_list_tree', 'reactor_event_loop').",
        ),
    ] = None,
    ...
    no_principles: Annotated[
        ...
        typer.Option(
            "--no-principles",
            help="Exclude quality principles and code smells (Patterns, Reactor & Safety only).",
        ),
    ] = False,
    exclude: Annotated[
        list[str] | None,
    ...
    """Scan a C codebase for OOP in C, memory patterns, reactor loops, and safety vulnerabilities."""
...
def _handle_terminal_scan(
    ...
    # 1. Header Banner
    header = Panel.fit(
        f"[bold white]⚡ DPX-C: Pure C Architecture & Design Pattern Detector • Hexagonal DDD[/bold white]\n"
    ...
    table = Table(title="📊 Detection Summary by Category", header_style="bold blue")
    ...
    console.print("[bold]📋 Identified Design Patterns & Memory Idioms:[/bold]\n")
    for idx, d in enumerate(report.detections, 1):
        ...
        header_text = f"[bold blue]#{idx} {d.pattern_type.value.upper()}[/bold blue] on [bold]{d.target_kind}[/bold] '[yellow]{d.target_name}[/yellow]'"
        ...
        for i, ev in enumerate(d.evidences):
            ...
            lines.append(f"{connector} [blue]+{int(ev.weight * 100)}%[/blue] ([dim]{ev.rule_code}[/dim]) {ev.description}{ev_loc}")
...
@app.command()
def rules() -> None:
    """List all supported C design patterns, memory idioms, and safety rules."""
    table = Table(title="⚡ Supported Patterns & Rules in DPX-C", header_style="bold blue")
...
@app.command()
def info() -> None:
    """Display DPX-C architecture, version, and grammar engine details."""
    panel = Panel(
        """[bold blue]⚡ DPX-C • Pure C Architecture & Design Pattern Detector[/bold blue]
[dim]Multi-Paradigm Hexagonal Pattern Detector & C AST/CST Engine[/dim]

• [bold]Target Language:[/bold] Pure C (C89, C99, C11, C17, C23)
• [bold]Architecture:[/bold] Hexagonal Ports & Adapters + Domain-Driven Design (DDD)
• [bold]Rules Supported:[/bold] 27 Rules (VTable, Opaque Pointer ADT, Intrusive list_head, Reactor epoll, Memory Safety)
