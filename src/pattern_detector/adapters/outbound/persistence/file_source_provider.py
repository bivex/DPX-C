"""File-system based C source code provider."""
    ...
    def get_sources(self, target_path: str, extensions: list[str] | None = None) -> dict[str, str]:
        ...
        if target.is_dir():
            for root, _, files in os.walk(target):
                if "/.git/" in root or "/build/" in root or "/out/" in root or "/vendor/" in root:
                    continue
                for file in files:
