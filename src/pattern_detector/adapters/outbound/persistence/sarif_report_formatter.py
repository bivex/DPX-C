"""OASIS SARIF v2.1.0 Report Formatter for C Pattern Detector."""
...
class SarifReportFormatter(ReportFormatterPort):
    ...
    def format(self, report: DetectionReport) -> str:
        ...
        for d in report.detections:
            rule_id = f"DPX-C-{d.pattern_type.value.upper()}"
            if rule_id not in rules_map:
                rules_map[rule_id] = {
                    "id": rule_id,
                    "name": d.pattern_type.value,
                    "shortDescription": {"text": f"C Pattern: {d.pattern_type.value.replace('_', ' ').title()}"},
            ...
            result_item: dict[str, Any] = {
                "ruleId": rule_id,
                "level": "error" if d.pattern_category.value in ("principle", "memory_safety") else "note",
