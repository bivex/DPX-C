...
class DuplicateCodeDryRule(BasePatternRule):
    """Detects duplicate function bodies in C source files."""
    ...
    def detect(self, model: CodeModel) -> list[Detection]:
        ...
        for f in model.all_files():
            for fn in f.functions.values():
                ...
                if len(clean_body) >= 60 and fn.name not in ("main", "init", "cleanup"):
                    h = hashlib.md5(clean_body.encode("utf-8")).hexdigest()
                    body_hashes.setdefault(h, []).append((f"{f.file_path}:{fn.name}", fn))
        ...
        for h, duplicates in body_hashes.items():
            if len(duplicates) >= 2:
                ...
                evidences = [
                    Evidence(
                        description=f"DRY Violation: Identical C function logic duplicated across {len(duplicates)} location(s): {', '.join(names[:3])}",
                        weight=0.80,
